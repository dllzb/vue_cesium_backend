CREATE TABLE IF NOT EXISTS `__PREFIX__cesiummapv_config` (
...
) ENGINE=InnoDB AUTO_INCREMENT=67 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='系统配置';

CREATE TABLE IF NOT EXISTS `__PREFIX__cesiummapv_lineseffect` (
  `id` int(10) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `type` enum('FlyLines','RoadPic','BusLines') NOT NULL DEFAULT 'FlyLines' COMMENT '类型:FlyLines=飞线,RoadPic=道路穿梭,BusLines=巴士穿梭',
  `width` float(4,2) NOT NULL DEFAULT '0.00' COMMENT '线宽度',
  `geojsonfile` varchar(255) NOT NULL DEFAULT '' COMMENT '数据信息',
  `effectimage` varchar(255) NOT NULL DEFAULT '' COMMENT '效果图片源',
  `setup_param` text NOT NULL COMMENT '具体参数',
  `showswitch` tinyint(1) NOT NULL DEFAULT '1' COMMENT '显示',
  `createtime` int(10) NOT NULL DEFAULT '0' COMMENT '创建时间',
  `updatetime` int(10) NOT NULL DEFAULT '0' COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='线坐标效果集合';

CREATE TABLE IF NOT EXISTS `__PREFIX__cesiummapv_mapimagery` (
  `id` int(10) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `name` varchar(50) NOT NULL DEFAULT '' COMMENT '底图名称',
  `type` enum('ArcGisMapServerImageryProvider','BingMapsImageryProvider','OpenStreetMapImageryProvider','TileMapServiceImageryProvider','GoogleEarthEnterpriseImageryProvider','GoogleEarthEnterpriseMapsProvider','GridImageryProvider','IonImageryProvider','MapboxImageryProvider','MapboxStyleImageryProvider','SingleTileImageryProvider','TileCoordinatesImageryProvider','UrlTemplateImageryProvider','WebMapServiceImageryProvider','WebMapTileServiceImageryProvider') NOT NULL DEFAULT 'ArcGisMapServerImageryProvider' COMMENT '图层类型:ArcGisMapServerImageryProvider=ArcGisMapServerImageryProvider,BingMapsImageryProvider=BingMapsImageryProvider,OpenStreetMapImageryProvider=OpenStreetMapImageryProvider,TileMapServiceImageryProvider=TileMapServiceImageryProvider,GoogleEarthEnterpriseImageryProvider=GoogleEarthEnterpriseImageryProvider,GoogleEarthEnterpriseMapsProvider=GoogleEarthEnterpriseMapsProvider,GridImageryProvider=GridImageryProvider,IonImageryProvider=IonImageryProvider,MapboxImageryProvider=MapboxImageryProvider,MapboxStyleImageryProvider=MapboxStyleImageryProvider,SingleTileImageryProvider=SingleTileImageryProvider,TileCoordinatesImageryProvider=TileCoordinatesImageryProvider,UrlTemplateImageryProvider=UrlTemplateImageryProvider,WebMapServiceImageryProvider=WebMapServiceImageryProvider,WebMapTileServiceImageryProvider=WebMapTileServiceImageryProvider',
  `classConfig` text NOT NULL COMMENT '实例化配置',
  `interfaceConfig` text NOT NULL COMMENT '接口配置',
  `offset` varchar(10) NOT NULL DEFAULT '0,0' COMMENT '坐标偏移x,y',
  `invertswitch` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否反色',
  `filterRGB` varchar(20) NOT NULL DEFAULT '#000000' COMMENT '滤镜颜色',
  `showswitch` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否显示',
  `weigh` int(10) NOT NULL DEFAULT '0' COMMENT '权重',
  `createtime` int(10) NOT NULL DEFAULT '0' COMMENT '创建时间',
  `updatetime` int(10) NOT NULL DEFAULT '0' COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8 COMMENT='底图层';

CREATE TABLE IF NOT EXISTS `__PREFIX__cesiummapv_modellist` (
  `id` int(10) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `name` varchar(255) NOT NULL DEFAULT '' COMMENT '名称',
  `url` varchar(200) NOT NULL COMMENT '模型地址',
  `showswitch` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否显示',
  `color` varchar(50) NOT NULL DEFAULT '' COMMENT '混合颜色',
  `colorModelist` enum('HIGHLIGHT','MIX','REPLACE') NOT NULL DEFAULT 'HIGHLIGHT' COMMENT '颜色模式:HIGHLIGHT=高亮,MIX=混合,REPLACE=替换',
  `duration` int(10) NOT NULL DEFAULT '20' COMMENT '旋转延迟',
  `rotateSpeed` int(5) NOT NULL DEFAULT '0' COMMENT '转速',
  `latRotation` float(5,3) NOT NULL DEFAULT '0.000' COMMENT '旋转(横向)',
  `positionLon` float(30,27) NOT NULL DEFAULT '0.000000000000000000000000000' COMMENT 'lon经度',
  `positionLat` float(30,27) NOT NULL DEFAULT '0.000000000000000000000000000' COMMENT 'lat纬度',
  `height` int(10) NOT NULL DEFAULT '1' COMMENT '高度',
  `scale` float(10,5) NOT NULL DEFAULT '1.00000' COMMENT '大小',
  `minimumPixelSize` int(10) NOT NULL DEFAULT '20' COMMENT '最小显示',
  `createtime` int(10) NOT NULL DEFAULT '0' COMMENT '创建时间',
  `updatetime` int(10) NOT NULL DEFAULT '0' COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='模型gltf管理';

CREATE TABLE IF NOT EXISTS `__PREFIX__cesiummapv_pointeffect` (
  `id` int(10) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `effect_type` set('CircleDiffusion','CircleScan','CircleWave','HexagonSpread','SpreadWall','Scanline') NOT NULL DEFAULT 'CircleDiffusion' COMMENT '效果类型:CircleDiffusion=圆扩散,CircleScan=雷达圆扫,CircleWave=水波纹,HexagonSpread=六边形扩散,SpreadWall=墙推扩散',
  `lon` float(12,9) NOT NULL DEFAULT '0.000000000' COMMENT '经度',
  `lat` float(12,9) NOT NULL DEFAULT '0.000000000' COMMENT '纬度',
  `height` int(10) NOT NULL DEFAULT '0' COMMENT '高度',
  `onswitch` tinyint(1) NOT NULL DEFAULT '1' COMMENT '显示效果',
  `color` varchar(30) NOT NULL DEFAULT '' COMMENT '颜色',
  `radius` int(10) NOT NULL DEFAULT '0' COMMENT '半径',
  `duration` int(10) NOT NULL DEFAULT '1000' COMMENT '延迟时间',
  `ext` varchar(255) NOT NULL DEFAULT '' COMMENT '扩展参数',
  `createtime` int(10) NOT NULL DEFAULT '0' COMMENT '创建时间',
  `updatetime` int(10) NOT NULL DEFAULT '0' COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COMMENT='点坐标效果集';

CREATE TABLE IF NOT EXISTS `__PREFIX__cesiummapv_titleset` (
  `id` int(10) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `name` varchar(200) NOT NULL DEFAULT '' COMMENT '名称',
  `url` varchar(255) NOT NULL DEFAULT '' COMMENT '地址',
  `offset_x` float NOT NULL DEFAULT '0' COMMENT '偏移量lon经度',
  `offset_y` float NOT NULL DEFAULT '0' COMMENT '偏移量lat维度',
  `offset_z` int(10) NOT NULL DEFAULT '0' COMMENT '偏移量高z米',
  `onswitch` tinyint(1) NOT NULL DEFAULT '1' COMMENT '是否启用',
  `flytoswitch` tinyint(1) NOT NULL DEFAULT '0' COMMENT '飞到该白膜',
  `color` varchar(50) NOT NULL DEFAULT '' COMMENT '白膜颜色',
  `effectswitch` tinyint(1) NOT NULL DEFAULT '1' COMMENT '开启打光效果',
  `height` int(10) NOT NULL DEFAULT '100' COMMENT '光环的高度范围',
  `effect_color` varchar(50) NOT NULL DEFAULT '' COMMENT '打光颜色',
  `createtime` int(10) NOT NULL DEFAULT '0' COMMENT '创建时间',
  `updatetime` int(10) NOT NULL DEFAULT '0' COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COMMENT='白膜列表';
